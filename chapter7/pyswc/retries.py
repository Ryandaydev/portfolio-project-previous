"""Code inspired by (https://speakeasyapi.dev)"""
                ...
                for code in retries.status_codes:
                    if "X" in code.upper():
                        ...
                        if res.status_code == parsed_code:
                            raise TemporaryError(res)
            except httpx.exceptions.ConnectionError as exception:
                if retries.config.retry_connection_errors:
                    raise

                raise PermanentError(exception) from exception
            except httpx.exceptions.Timeout as exception:
                if retries.config.retry_connection_errors:
                    raise
...
def retry_with_backoff(func, initial_interval=500, max_interval=60000, exponent=2, max_elapsed_time=30000):
